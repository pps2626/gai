...
interface AppContextState {
  ...
  getUnreadGlobalCount: () => number;
  getUnreadDirectMessageCount: (partnerId: string) => number;
  getTotalUnreadDirectMessageCount: () => number;
}
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = (username: string, password: string): boolean => {
    ...
    if (user) {
      setCurrentUser(user);
      setLastReadTimestamps({}); // Reset on login
      return true;
    }
    return false;
  };
  ...
  const getUnreadGlobalCount = useCallback(() => {
    if (!currentUser) return 0;
    const lastRead = lastReadTimestamps['global'] || 0;
    return chatMessages.filter(msg => msg.timestamp > lastRead && msg.senderId !== currentUser.id).length;
  }, [chatMessages, currentUser, lastReadTimestamps]);

  const getUnreadDirectMessageCount = useCallback((partnerId: string) => {
    if (!currentUser) return 0;
    const lastRead = lastReadTimestamps[partnerId] || 0;
    return privateChatMessages.filter(
      msg => msg.senderId === partnerId && msg.receiverId === currentUser.id && msg.timestamp > lastRead
    ).length;
  }, [privateChatMessages, currentUser, lastReadTimestamps]);

  const getTotalUnreadDirectMessageCount = useCallback(() => {
    if (!currentUser) return 0;
    return users
      .filter(u => u.id !== currentUser.id)
      .reduce((total, user) => total + getUnreadDirectMessageCount(user.id), 0);
  }, [users, currentUser, getUnreadDirectMessageCount]);
  ...
  const value = {
    ...
    getTotalUnreadDirectMessageCount
  };
